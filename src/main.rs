...
enum Commands {
    ...
    /// Launch a specific application in a VM via Waypipe
    Launch {
        /// VM name
        name: String,
...
async fn create_vm(
    ...
    println!(
        "   Passwords: {}/.config/vm-provisioner/vm-passwords.toml",
        std::env::var("HOME")?
    );
...
async fn start_vm(name: String) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // For headless VMs, skip Waypipe instructions and SPICE viewer
    if config.headless {
    ...
    // Display information about Waypipe integration
    println!("\n🪟 Seamless Window Integration via Waypipe");
    println!("   Waypipe connects via SSH (passwordless) with Zstd compression");
    println!("   Use commands:");
    println!(
        "     vm-provisioner generate-shortcuts {}  # Create .desktop files",
        name
    );
    println!(
        "     vm-provisioner launch {} <app>       # Launch specific app",
        name
    );
    println!(
        "     vm-provisioner apps {}               # List available apps",
        name
    );

    if config.enable_clipboard {
        println!("\n📋 Clipboard sharing enabled via Waypipe (Wayland protocol)");
    }
...
async fn destroy_vm(name: String, skip_confirm: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let config_file = if Path::new(&user_config).exists() {
        ...
        eprintln!(
            "   Try running: sudo ./target/release/vm-provisioner destroy {} --yes",
            name
        );
        std::process::exit(1);
    };
    ...
    if let Ok(waypipe_manager) = WaypipeManager::new(&config) {
        if let Err(e) = waypipe_manager.remove_desktop_files() {
...
async fn generate_shortcuts(name: String) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create Waypipe manager and generate shortcuts
    let waypipe_manager = WaypipeManager::new(&config)?;
    waypipe_manager.generate_desktop_files()?;
...
async fn launch_app(name: String, app: String) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create Waypipe manager and launch app
    let waypipe_manager = WaypipeManager::new(&config)?;
    waypipe_manager.launch_app(&app)?;
...
fn list_apps(name: String) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create Waypipe manager and list apps
    let waypipe_manager = match WaypipeManager::new(&config) {
        Ok(manager) => manager,
        Err(e) => {
    ...
    let apps = waypipe_manager.list_applications();
